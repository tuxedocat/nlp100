...
if __name__ == '__main__':
    get_leftmostverb = get_leftmostxx_func("動詞", ifnone="")
    get_particle = get_leftmostxx_func("助詞", ifnone="")
    ...
    for sent in parsed:
        for srcc in sent:
            ...
            if is_sahen_vp(srcc.morphs):
                ...
                if vp:
                    particles = [
                        get_particle(ph.morphs) for i, ph in enumerate(sent) if ph.dst == vp.srcs[0]]
                    words = [ph.chunktext(filter_p=punct_p) for i, ph in enumerate(
                        sent) if ph.dst == vp.srcs[0]]
                    if particles and words:
                        predcases.append((srcc.chunktext_vbase(
                            filter_p=punct_p) + get_leftmostverb(vp.morphs),                         zip(particles, words)))
